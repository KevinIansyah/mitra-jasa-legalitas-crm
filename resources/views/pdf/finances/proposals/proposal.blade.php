    ...
    <div>
      ...
      <div class="mt-10 grid grid-cols-2 gap-8">
        <div>
          ...
          <p class="text-sm font-semibold text-zinc-900">{{ $proposal->customer?->name ?? '-' }}</p>
          <div class="text-sm text-zinc-600">
            @if ($proposal->customer?->email)
              <p>{{ $proposal->customer->email }}</p>
            @endif
            @if ($proposal->customer?->phone)
              <p>{{ $proposal->customer->phone }}</p>
            @endif
          </div>
          @if ($proposal->project_name)
            <p class="mt-2 text-xs text-zinc-600">Project: <span
                class="font-medium text-zinc-900">{{ $proposal->project_name }}</span></p>
          @endif
        </div>
        <div>
          <p class="mb-2 text-xs font-semibold uppercase tracking-widest text-zinc-600">Detail Proposal</p>
          <div class="space-y-1 text-sm">

            <div class="flex justify-between gap-4">
              <span class="text-zinc-600">Tanggal</span>
              <span class="font-medium text-zinc-900">{{ $proposal->proposal_date->format('d M Y') }}</span>
            </div>
            @if ($proposal->valid_until)
              <div class="flex justify-between gap-4">
                <span class="text-zinc-600">Berlaku Hingga</span>
                <span class="font-medium text-zinc-900">{{ $proposal->valid_until->format('d M Y') }}</span>
              </div>
            @endif
      ...
      <div class="mt-6 flex justify-end no-break">
        <div class="w-72 space-y-2 text-sm">
          <div class="flex justify-between">
            ...
            <span class="text-zinc-900">Rp {{ number_format($proposal->subtotal, 0, ',', '.') }}</span>
          </div>
          @if ($proposal->discount_amount > 0)
            <div class="flex justify-between">
              <span class="text-zinc-600">Diskon ({{ $proposal->discount_percent }}%)</span>
              <span class="text-zinc-900">-Rp {{ number_format($proposal->discount_amount, 0, ',', '.') }}</span>
            </div>
          @endif
          @if ($proposal->tax_amount > 0)
            <div class="flex justify-between">
              <span class="text-zinc-600">Pajak ({{ $proposal->tax_percent }}%)</span>
      ...
      @if ($proposal->notes)
        <div class="mt-6 no-break">
          ...
          <p class="whitespace-pre-line text-sm text-zinc-600">{{ $proposal->notes }}</p>
        </div>
      @endif
