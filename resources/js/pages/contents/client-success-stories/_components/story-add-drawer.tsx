...
import type { ClientSuccessStoryFormData } from '@/types/contents';

export function StoryAddDrawer() {
    ...
    const { data, setData, post, processing, errors, reset, clearErrors } = useForm<ClientSuccessStoryFormData>({
        client_name: '',
        industry: '',
        client_logo: null,
        metric_value: '',
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        const toastId = toast.loading('Menyimpan...', { description: 'Kisah sukses sedang ditambahkan.' });

        post(contents.clientSuccessStories.store.url(), {
            forceFormData: true,
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Berhasil', { description: 'Kisah sukses berhasil ditambahkan.' });
                reset();
                setOpen(false);
            },
            onError: () => {
            ...
            <DrawerContent className="flex h-screen flex-col">
                <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-4 overflow-y-auto">
                    <DrawerHeader className="px-4">
                        <DrawerTitle>Tambah Kisah Sukses</DrawerTitle>
                        <DrawerDescription>Profil klien, industri (sesuai kategori bisnis kontak), metrik utama, cerita, dan logo opsional.</DrawerDescription>
                    </DrawerHeader>
