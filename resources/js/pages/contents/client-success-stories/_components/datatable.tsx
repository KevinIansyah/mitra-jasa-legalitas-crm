...
export function DataTable({ data, pageIndex, setPageIndex, totalPages, totalItems, perPage, initialFilters = {} }: DataTableProps) {
    ...
    const { searchValue, clearSearch, handleSearchChange, filters, updateFilter, goToPage, changePageSize, canPreviousPage, canNextPage, resetFilters, activeFiltersCount } =
        useDataTableWithFilters<StoryFilters>({
            pageIndex,
            setPageIndex,
            totalPages,
            perPage,
            initialFilters,
            onlyFields: ['stories', 'filters', 'summary'],
            routeUrl: contents.clientSuccessStories.index().url,
        });

    const columns = React.useMemo(() => getColumns(), []);

    const selectedPublished = CLIENT_SUCCESS_PUBLISHED_FILTERS.find((f) => f.value === filters.published);
    const selectedIndustry = CATEGORY_BUSINESS.find((c) => c.value === filters.industry);
            ...
            <div className="flex flex-col gap-4 pb-4">
                <div className="flex flex-col items-center justify-between gap-2 md:flex-row">
                    <div className="flex w-full flex-1 items-center gap-2 md:w-auto">
                        <InputGroup className="max-w-sm">
                            <InputGroupInput placeholder="Cari nama klien / label metrik..." value={searchValue} onChange={handleSearchChange} />
                            <InputGroupAddon>
                                <Search />
                            </InputGroupAddon>
                        </InputGroup>
                        ...
                        <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                            ...
                            <SheetContent>
                                <SheetHeader>
                                    ...
                                    <SheetDescription>Status publikasi atau industri</SheetDescription>
                                </SheetHeader>
                                <div className="space-y-4 px-4">
                                        ...
                                        <Select value={filters.published || ''} onValueChange={(v) => updateFilter('published', v || undefined)}>
                                            ...
                                            <SelectContent>
                                                <SelectGroup>
                                                    ...
                                                    {CLIENT_SUCCESS_PUBLISHED_FILTERS.map((f) => (
                                    ...
                                    <Field>
                                        ...
                                        <Select value={filters.industry || ''} onValueChange={(v) => updateFilter('industry', v || undefined)}>
                                            <SelectTrigger className="w-full">
                                                <SelectValue placeholder="Semua industri" />
                                            </SelectTrigger>
                                            <SelectContent>
                                                <SelectGroup>
                                                    <SelectLabel>Industri</SelectLabel>
                    ...
                    <div className="flex w-full gap-2 md:w-auto">
                        ...
                        <HasPermission permission="create-content-client-success-stories">
                            <StoryAddDrawer />
                        </HasPermission>
                    </div>
                </div>
                ...
                {activeFiltersCount > 0 && (
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {filters.industry && (
                            <Badge variant="secondary" className="gap-2">
                                Industri: {selectedIndustry?.label}
                                <Button variant="ghost" size="sm" className="h-6 w-6 text-xs" onClick={() => updateFilter('industry', undefined)}>
                                    <X className="size-3" />
                                </Button>
                            </Badge>
                        )}
            ...
            <div className="flex items-center justify-between gap-8 pt-4">
                ...
                <div className="flex w-full items-center gap-8 md:w-fit">
                    <div className="hidden items-center gap-2 md:flex">
                        <Label htmlFor="rows-css" className="text-sm font-medium">
                            Baris per halaman
                        </Label>
                        <Select value={`${perPage}`} onValueChange={(v) => changePageSize(Number(v))}>
                            <SelectTrigger className="w-20" id="rows-css">
