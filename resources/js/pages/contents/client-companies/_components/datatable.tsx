...
import { CLIENT_COMPANY_PUBLISHED_FILTERS } from '@/types/contents';
import { ClientCompanyAddDrawer } from './client-company-add-drawer';
import getColumns from './columns';

type ClientCompanyFilters = {
    search?: string;
    published?: string;
};
...
type DataTableProps = {
    ...
    totalItems: number;
    perPage: number;
    initialFilters?: ClientCompanyFilters;
}
...
export function DataTable({ data, pageIndex, setPageIndex, totalPages, totalItems, perPage, initialFilters = {} }: DataTableProps) {
    ...
    const { searchValue, clearSearch, handleSearchChange, filters, updateFilter, goToPage, changePageSize, canPreviousPage, canNextPage, resetFilters, activeFiltersCount } =
        useDataTableWithFilters<ClientCompanyFilters>({
            pageIndex,
            setPageIndex,
            totalPages,
            perPage,
            initialFilters,
            onlyFields: ['client_companies', 'filters', 'summary'],
            routeUrl: contents.clientCompanies.index().url,
        });

    const columns = React.useMemo(() => getColumns(), []);

    const selectedPublished = CLIENT_COMPANY_PUBLISHED_FILTERS.find((f) => f.value === filters.published);
                ...
                <div className="flex flex-col items-center justify-between gap-2 md:flex-row">
                    <div className="flex w-full flex-1 items-center gap-2 md:w-auto">
                        ...
                        <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                            ...
                            <SheetContent>
                                <SheetHeader>
                                    <SheetTitle>Filter Logo Klien</SheetTitle>
                                    <SheetDescription>Status publikasi</SheetDescription>
                                </SheetHeader>
                                        ...
                                        <Select value={filters.published || ''} onValueChange={(v) => updateFilter('published', v || undefined)}>
                                            ...
                                            <SelectContent>
                                                <SelectGroup>
                                                    ...
                                                    {CLIENT_COMPANY_PUBLISHED_FILTERS.map((f) => (
                    ...
                    <div className="flex w-full gap-2 md:w-auto">
                        ...
                        <HasPermission permission="create-content-client-companies">
                            <ClientCompanyAddDrawer />
                        </HasPermission>
                    </div>
                </div>
            ...
            <div className="flex items-center justify-between gap-8 pt-4">
                ...
                <div className="flex w-full items-center gap-8 md:w-fit">
                    <div className="hidden items-center gap-2 md:flex">
                        <Label htmlFor="rows-per-page-cc" className="text-sm font-medium">
                            Baris per halaman
                        </Label>
                        <Select value={`${perPage}`} onValueChange={(v) => changePageSize(Number(v))}>
                            <SelectTrigger className="w-20" id="rows-per-page-cc">
