...
import type { Testimonial, TestimonialFormData } from '@/types/contents';
...
type TestimonialEditDrawerProps = {
...
export function TestimonialEditDrawer({ testimonial, services, open, onOpenChange }: TestimonialEditDrawerProps) {
    ...
    const [filePreview, setFilePreview] = React.useState<{ src?: string; name: string; size: number; isImage: boolean } | null>(
        testimonial.client_avatar
            ? {
                  src: `${R2_PUBLIC_URL}/${testimonial.client_avatar}`,
                  name: testimonial.client_avatar.split('/').pop() || 'file',
                  size: 0,
                  isImage: /\.(jpg|jpeg|png|webp)$/i.test(testimonial.client_avatar),
              }
            : null,
    );
    ...
    const { data, setData, post, processing, errors } = useForm<TestimonialFormData>({
        ...
        is_published: testimonial.is_published ? 1 : 0,
        remove_client_avatar: false,
    });
    ...
    const handleFile = async (file: File | undefined) => {
        ...
        setData('remove_client_avatar', false);
    ...
    const handleRemoveFile = () => {
        ...
        setData('remove_client_avatar', true);
        if (fileInputRef.current) fileInputRef.current.value = '';
    };
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        const toastId = toast.loading('Menyimpan...', { description: 'Testimoni sedang diperbarui.' });

        post(contents.testimonials.update(testimonial.id).url, {
            forceFormData: true,
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Berhasil', { description: 'Testimoni berhasil diperbarui.' });
                onOpenChange(false);
            },
            onError: () => {
                toast.error('Gagal', { description: 'Terjadi kesalahan saat memperbarui testimoni, coba lagi.' });
            },
            onFinish: () => {
            ...
            >
                <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-4 overflow-y-auto">
                    <DrawerHeader className="px-4">
                        ...
                        <DrawerDescription>Perbarui data testimoni. Unggah foto baru untuk mengganti avatar.</DrawerDescription>
                    </DrawerHeader>
