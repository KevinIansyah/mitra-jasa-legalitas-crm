...
import { TESTIMONIAL_PUBLISHED_FILTERS } from '@/types/contents';
import type { Service } from '@/types/services';
import getColumns from './columns';
import { TestimonialAddDrawer } from './testimonial-add-drawer';
...
export function DataTable({ data, services, pageIndex, setPageIndex, totalPages, totalItems, perPage, initialFilters = {} }: DataTableProps) {
    ...
    const { searchValue, clearSearch, handleSearchChange, filters, updateFilter, goToPage, changePageSize, canPreviousPage, canNextPage, resetFilters, activeFiltersCount } =
        useDataTableWithFilters<TestimonialFilters>({
            pageIndex,
            setPageIndex,
            totalPages,
            perPage,
            initialFilters,
            onlyFields: ['testimonials', 'filters', 'summary', 'services'],
            routeUrl: contents.testimonials.index().url,
        });

    const columns = React.useMemo(() => getColumns(services), [services]);

    const selectedPublished = TESTIMONIAL_PUBLISHED_FILTERS.find((f) => f.value === filters.published);
    const selectedService = services.find((s) => String(s.id) === filters.service_id);
                ...
                <div className="flex flex-col items-center justify-between gap-2 md:flex-row">
                    <div className="flex w-full flex-1 items-center gap-2 md:w-auto">
                        ...
                        <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                            ...
                            <SheetContent>
                                <SheetHeader>
                                    <SheetTitle>Filter Testimoni</SheetTitle>
                                    <SheetDescription>Status publikasi atau layanan terkait</SheetDescription>
                                </SheetHeader>
                                <div className="space-y-4 px-4">
                                    <Field>
                                        <FieldLabel>Status</FieldLabel>
                                        <Select value={filters.published || ''} onValueChange={(v) => updateFilter('published', v || undefined)}>
                                            ...
                                            <SelectContent>
                                                <SelectGroup>
                                                    ...
                                                    {TESTIMONIAL_PUBLISHED_FILTERS.map((f) => (
                                    ...
                                    <Field>
                                        ...
                                        <Select value={filters.service_id || ''} onValueChange={(v) => updateFilter('service_id', v || undefined)}>
                                            <SelectTrigger className="w-full">
                    ...
                    <div className="flex w-full gap-2 md:w-auto">
                        ...
                        <HasPermission permission="create-content-testimonials">
                            <TestimonialAddDrawer services={services} />
                        </HasPermission>
                    </div>
                </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {filters.published && (
                            <Badge variant="secondary" className="gap-2">
                                Status: {selectedPublished?.label}
                                <Button variant="ghost" size="sm" className="h-6 w-6 text-xs" onClick={() => updateFilter('published', undefined)}>
                                    <X className="size-3" />
                                </Button>
                            </Badge>
                        )}
                        {filters.service_id && (
                            <Badge variant="secondary" className="gap-2">
                                ...
                                <Button variant="ghost" size="sm" className="h-6 w-6 text-xs" onClick={() => updateFilter('service_id', undefined)}>
                                    <X className="size-3" />
                                </Button>
                            </Badge>
                        )}
            ...
            <div className="flex items-center justify-between gap-8 pt-4">
                ...
                <div className="flex w-full items-center gap-8 md:w-fit">
                    <div className="hidden items-center gap-2 md:flex">
                        <Label htmlFor="rows-per-page-t" className="text-sm font-medium">
                            Baris per halaman
                        </Label>
                        <Select value={`${perPage}`} onValueChange={(v) => changePageSize(Number(v))}>
                            <SelectTrigger className="w-20" id="rows-per-page-t">
