...
export function DrawerEdit({ tag, open, onOpenChange }: DrawerEditProps) {
    const loadingFocusRef = React.useRef<HTMLButtonElement>(null);

    const { data, setData, patch, processing, errors } = useForm<BlogTagFormData>({
        name: tag.name || '',
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const id = toast.loading('Memproses...', {
            description: 'Tag blog sedang diperbarui.',
        });

        patch(tags.update(tag.id).url, {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Berhasil', {
                    description: 'Tag blog berhasil diperbarui.',
                });
            ...
            onError: () => {
                toast.error('Gagal', {
                    description: 'Tag blog gagal diperbarui. Silakan periksa kembali data yang diisi.',
                });
            },
            onFinish: () => {
                toast.dismiss(id);
