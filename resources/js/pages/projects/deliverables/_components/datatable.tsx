...
import { ChevronDown, ChevronLeftIcon, ChevronRightIcon, ChevronsLeftIcon, ChevronsRightIcon, Search, X } from 'lucide-react';
...
export function DataTable({ data, pageIndex, setPageIndex, totalPages, totalItems, perPage, initialFilters = {} }: DataTableProps) {
    ...
    const { searchValue, clearSearch, handleSearchChange, goToPage, changePageSize, canPreviousPage, canNextPage, resetFilters, activeFiltersCount } = useDataTableWithFilters({
        ...
        perPage,
        initialFilters,
        onlyFields: ['deliverables', 'filters'],
        routeUrl: deliverablesIndex().url,
    });
                                        ...
                                        <TableRow className="hover:bg-transparent">
                                            <TableCell colSpan={columns.length} className="pb-4">
                                                <div className="rounded-lg bg-primary/10 dark:bg-muted/40">
                                                    <DeliverableDetail deliverable={row.original} />
                                                </div>
                                            </TableCell>
                                        </TableRow>
                                    )}
