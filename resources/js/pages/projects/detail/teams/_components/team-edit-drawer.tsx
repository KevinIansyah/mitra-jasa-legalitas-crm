...
import type { MemberRole, ProjectMember, ProjectMemberFormData } from '@/types/project';
import { MEMBER_ROLES } from '@/types/project';

type TeamEditDrawerProps = {
    projectId: number;
    team: ProjectMember;
    open: boolean;
...
export function TeamEditDrawer({ projectId, team, open, onOpenChange }: TeamEditDrawerProps) {
    const [searchQuery, setSearchQuery] = React.useState('');
    const [searchResults, setSearchResults] = React.useState<User[]>([]);
    const [isSearching, setIsSearching] = React.useState(false);
    const [selectedUser, setSelectedUser] = React.useState<User | null>(null);

    const { data, setData, put, processing, errors, reset } = useForm<ProjectMemberFormData>({
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const id = toast.loading('Memproses...', {
            description: 'Anggota tim sedang diperbarui.',
        });

        put(projects.teams.update({ project: projectId, member: team.id }).url, {
            preserveScroll: true,
            onSuccess: () => {
            ...
            <DrawerContent className="flex h-screen flex-col">
                <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-4 overflow-y-auto">
                    <DrawerHeader className="px-4">
                        ...
                        <DrawerDescription>Perbarui data invoice yang sudah ada melalui formulir di bawah ini.</DrawerDescription>
                    </DrawerHeader>
