...
interface DataTableProps {
    ...
    totalItems: number;
    perPage: number;
    initialFilters?: { search?: string; category?: string; status?: string };
}
...
export function DataTable({ data, pageIndex, setPageIndex, totalPages, totalItems, perPage, initialFilters = {} }: DataTableProps) {
    ...
    const { searchValue, clearSearch, handleSearchChange, filters, updateFilter, goToPage, changePageSize, canPreviousPage, canNextPage, resetFilters, activeFiltersCount } =
        useDataTableWithFilters({
            ...
            perPage,
            initialFilters,
            onlyFields: ['vendors', 'filters'],
            routeUrl: finances.vendors.index().url,
        });

    const columns = getColumns(expandedRow, setExpandedRow);
    const selectedCategory = VENDOR_CATEGORIES.find((c) => c.value === filters.category);
                ...
                <div className="flex flex-col items-center justify-between gap-2 md:flex-row">
                    <div className="flex w-full flex-1 items-center gap-2 md:w-auto">
                        <InputGroup className="max-w-sm">
                            <InputGroupInput placeholder="Cari nama, email, telepon..." value={searchValue} onChange={handleSearchChange} />
                            <InputGroupAddon>
                                <Search />
                            </InputGroupAddon>
                        </InputGroup>
                    ...
                    <div className="flex w-full gap-2 md:w-auto">
                        ...
                        {/* Add Vendor */}
                        <HasPermission permission="create-finance-vendors">
                            <Button className="flex-1 gap-1.5 md:w-30" asChild>
                                <Link href={finances.vendors.create().url}>
                                    <Plus />
                                    Tambah
                                </Link>
                            </Button>
                                        ...
                                        <TableRow className="hover:bg-transparent">
                                            <TableCell colSpan={columns.length} className="pb-4">
                                                <div className="rounded-lg bg-primary/10 dark:bg-muted/40">
                                                    <VendorDetail vendor={row.original} />
                                                </div>
                                            </TableCell>
                                        </TableRow>
                                    )}
