...
export function DataTable({ data, pageIndex, setPageIndex, totalPages, totalItems, perPage, initialFilters = {} }: DataTableProps) {
    ...
    const { searchValue, clearSearch, handleSearchChange, filters, updateFilter, goToPage, changePageSize, canPreviousPage, canNextPage, resetFilters, activeFiltersCount } =
        useDataTableWithFilters({
            ...
            perPage,
            initialFilters,
            onlyFields: ['proposals', 'filters'],
            routeUrl: finances.proposals.index().url,
        });

    const columns = getColumns(expandedRow, setExpandedRow);
    const selectedStatus = PROPOSAL_STATUSES.find((s) => s.value === filters.status);
                ...
                <div className="flex flex-col items-center justify-between gap-2 md:flex-row">
                    <div className="flex w-full flex-1 items-center gap-2 md:w-auto">
                        <InputGroup className="max-w-sm">
                            <InputGroupInput placeholder="Cari nomor proposal / customer..." value={searchValue} onChange={handleSearchChange} />
                            <InputGroupAddon>
                                <Search />
                            </InputGroupAddon>
                        </InputGroup>
                        ...
                        <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                            ...
                            <SheetContent>
                                <SheetHeader>
                                    <SheetTitle>Filter Proposal</SheetTitle>
                                    <SheetDescription>Saring data berdasarkan status proposal</SheetDescription>
                                </SheetHeader>
                    ...
                    <div className="flex w-full gap-2 md:w-auto">
                        ...
                        <HasPermission permission="create-finance-proposals">
                            <Button className="flex-1 gap-1.5 md:w-30" asChild>
                                <Link href={finances.proposals.create().url}>
                                    <Plus />
                                    Tambah
                                </Link>
                            </Button>
                                        ...
                                        <TableRow className="hover:bg-transparent">
                                            <TableCell colSpan={columns.length} className="pb-4">
                                                <div className="rounded-lg bg-primary/10 dark:bg-muted/40">
                                                    <ProposalDetail proposal={row.original} />
                                                </div>
                                            </TableCell>
                                        </TableRow>
                                    )}
