...
type DataTableProps = {
    ...
    totalItems: number;
    perPage: number;
    initialFilters?: { search?: string; status?: string; timeline?: string; source?: string };
}
...
export function DataTable({ data, pageIndex, setPageIndex, totalPages, totalItems, perPage, initialFilters = {} }: DataTableProps) {
    ...
    const { searchValue, clearSearch, handleSearchChange, filters, updateFilter, goToPage, changePageSize, canPreviousPage, canNextPage, resetFilters, activeFiltersCount } =
        useDataTableWithFilters({
            ...
            perPage,
            initialFilters,
            onlyFields: ['quotes', 'filters'],
            routeUrl: finances.quotes.index().url,
        });

    const columns = getColumns(expandedRow, setExpandedRow);

    const selectedStatus = QUOTE_STATUSES.find((s) => s.value === filters.status);
    const selectedTimeline = QUOTE_TIMELINES.find((t) => t.value === filters.timeline);
    const selectedSource = QUOTE_SOURCES.find((s) => s.value === filters.source);
                ...
                <div className="flex flex-col items-center justify-between gap-2 md:flex-row">
                    <div className="flex w-full flex-1 items-center gap-2 md:w-auto">
                        ...
                        <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                            ...
                            <SheetContent>
                                <SheetHeader>
                                    <SheetTitle>Filter Quote</SheetTitle>
                                    <SheetDescription>Saring data quote berdasarkan status, timeline, atau sumber</SheetDescription>
                                </SheetHeader>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {filters.timeline && (
                            <Badge variant="secondary" className="gap-2">
                                ...
                                <Button variant="ghost" size="sm" className="h-6 w-6 text-xs" onClick={() => updateFilter('timeline', undefined)}>
                                    <X className="size-3" />
                                </Button>
                            </Badge>
                        )}
                                        ...
                                        <TableRow className="hover:bg-transparent">
                                            <TableCell colSpan={columns.length} className="pb-4">
                                                <div className="rounded-lg bg-primary/10 dark:bg-muted/40">
                                                    <QuoteDetail quote={row.original} />
                                                </div>
                                            </TableCell>
                                        </TableRow>
                                    )}
