...
import type { JournalLineFormData, ManualJournalFormData, ManualJournalFormErrors } from '@/types/journal-entries';
...
export function JournalAddDrawer({ open, onOpenChange, accounts }: Props) {
    ...
    const handleSubmit = () => {
        const toastId = toast.loading('Memproses...', { description: 'Jurnal sedang disimpan.' });

        post(finances.journalEntries.store().url, {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Berhasil', { description: 'Jurnal berhasil disimpan.' });
                reset();
                onOpenChange(false);
            },
            onError: (errs) => {
            ...
            <DrawerContent className="flex h-screen flex-col">
                <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col overflow-y-auto">
                    <DrawerHeader className="px-4">
                        <DrawerTitle>Tambah Jurnal Manual</DrawerTitle>
                        <DrawerDescription>Catat transaksi penyesuaian atau koreksi. Total debit harus sama dengan total kredit.</DrawerDescription>
                    </DrawerHeader>
                    ...
                    <DrawerFooter className="px-4">
                        <Button onClick={handleSubmit} disabled={processing || !isBalanced || !data.date || !data.description.trim()}>
                            {processing ? (
                                <>
