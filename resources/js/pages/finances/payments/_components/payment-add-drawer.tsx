...
export function PaymentAddDrawer({ invoice, open, onOpenChange }: PaymentAddDrawerProps) {
    ...
    const { data, setData, post, processing, errors, reset } = useForm<ProjectPaymentFormData>({
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        post(finances.invoices.payments.store(invoice.id).url, {
            ...
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Berhasil', { description: 'Pembayaran berhasil ditambahkan.' });
                reset();
                setFilePreview(null);
                onOpenChange(false);
            },
            ...
            <DrawerContent className="flex h-screen flex-col">
                <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-4 overflow-y-auto">
                    <DrawerHeader className="px-4">
                        <DrawerTitle>Tambah Pembayaran</DrawerTitle>
                        <DrawerDescription>Isi formulir untuk menambahkan pembayaran invoice ini.</DrawerDescription>
                    </DrawerHeader>
