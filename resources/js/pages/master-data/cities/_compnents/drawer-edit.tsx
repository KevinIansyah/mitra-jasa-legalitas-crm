...
export function DrawerEdit({ city, open, onOpenChange }: DrawerEditProps) {
    const loadingFocusRef = React.useRef<HTMLButtonElement>(null);

    const { data, setData, patch, processing, errors } = useForm<CityFormData>({
        name: city.name || '',
        slug: city.slug || '',
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const id = toast.loading('Memproses...', {
            description: 'Kota sedang diperbarui.',
        });

        patch(citiesRoute.update(city.id).url, {
            preserveScroll: true,
            onSuccess: () => {
            ...
            onError: (errors) => {
                const msg = Object.values(errors)[0] ?? 'Terjadi kesalahan saat memperbarui kota, coba lagi.';
            ...
            >
                <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-4 overflow-y-auto">
                    <DrawerHeader className="px-4">
                        <DrawerTitle>Edit Kota</DrawerTitle>
                        <DrawerDescription>Perbarui data kota yang sudah ada melalui formulir di bawah ini.</DrawerDescription>
                    </DrawerHeader>
