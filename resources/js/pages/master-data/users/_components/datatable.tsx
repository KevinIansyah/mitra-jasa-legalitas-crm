...
export function DataTable({ data, pageIndex, setPageIndex, totalPages, totalItems, perPage, initialFilters = {} }: DataTableProps) {
    ...
    const { searchValue, clearSearch, handleSearchChange, filters, updateFilter, goToPage, changePageSize, canPreviousPage, canNextPage, resetFilters, activeFiltersCount } =
        useDataTableWithFilters({
            ...
            perPage,
            initialFilters,
            onlyFields: ['users', 'filters'],
            routeUrl: '/master-data/users',
        });
    ...
    const selectedStatus = USER_STATUS.find((s) => s.value === filters.status);
                ...
                <div className="flex flex-col items-center justify-between gap-2 md:flex-row">
                    <div className="flex w-full flex-1 items-center gap-2 md:w-auto">
                        <InputGroup className="max-w-sm">
                            <InputGroupInput placeholder="Cari nama, email, atau telepon..." value={searchValue} onChange={handleSearchChange} />
                            <InputGroupAddon>
                                <Search />
                            </InputGroupAddon>
                        </InputGroup>
                        ...
                        <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                            ...
                            <SheetContent>
                                <SheetHeader>
                                    ...
                                    <SheetDescription>Saring data berdasarkan status akun</SheetDescription>
                                </SheetHeader>
                        ...
                        {table.getRowModel().rows?.length ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={columns.length} className="h-24 text-center">
                                    {searchValue ? 'Tidak ada hasil yang ditemukan' : 'Tidak ada data user'}
                                </TableCell>
                            </TableRow>
                        )}
                    </TableBody>
